...
- let
  ...
  計算式を実行し結果を得ます。  
  この命令はB/C/IX以外のレジスタを使用して計算式を実行し、それ以外のレジスタは破壊されます。
  ```
  // 普通の式
  B = 1;
  C = 5;
  ...
  // アドレスの取得
  B = 1;
  byte[2][2] buf = {{0,1},{2,3}};
  let HL = &buf[B][0]; // 配列のサイズが256バイト以内におさまる場合、添え字に16bit値は使えません。
  A = *HL;             // A = 2;
  ...
  let文は代入先は8bitの場合は8bit式、16bitの場合は16bit式として判断し使用されるレジスタが変わります。  
  最適化により必ずしもこの通りにならないことがありますが、破壊されるレジスタの目安にしてください。
